...
SAFE enables users (named Alice in this document) to quickly move tokens on an `FromChain` to an `ToChain`, utilizing a liquidity provider (named Bob in this document) who holds liquidity on `ToChain`. Largest costs savings are where where `FromChain` transactions are much more expensive than `ToChain` transactions, minimizing cost with the following design properties:

- Alice only needs to submit a single transaction to `FromChain`. (In particular, if Alice goes offline at any point during the process, the system can continue smoothly. **_This is important, since Alice is expected to be a regular user and may drop her cell phone in the ocean._**)
- To service a swap for Alice, Bob must submit two `FromChain` transactions plus one `ToChain` transactions. However, Bob may service a batch of `n` swaps with this triplet of transactions, amortizing the bulk of the cost across many swaps.

Thus, Alice's swap is serviced with `1 + 2/n` transactions on `FromChain` and `1/n` transactions on `ToChain`, where `n` is the number of swaps serviced per batch. (This is slightly inaccurate, since it ignores transactions required by Bob to move liquidity from `FromChain` back to `ToChain` and into the holdings contract. Liquidity moves are not required for each batch, so this inaccuracy is likely to be a rounding error.)

_**Note:** The protocol is inspired by optimizing for a specific use case, where `ToChain` is mainnet Ethereum ("Layer 1", or L1), and `FromChain` is an optimistic roll-up (ORU) Layer 2, or L2. Users who want to withdraw funds from an ORU L2 must wait an extended period of time before accessing their funds on L1._

1. Alice deposits `x` tokens on `FromChain`, and is given a ticket `t`. The ticket is initially in the `pending` state and records that "`x` tokens should be sent to Alice on `ToChain`."
2. Bob then authorizes `t` on the `FromChain` by including it in a signed batch `b` and calling `authorizeFromChainBatch(b)`. The ticket is now in the `authorized` state.
3. During normal operation, Bob will submit `b` to `ToChain`. If Bob fails to do so, Alice _has the ability_ to submit `b`. Submission of `b` sends `x` tokens to Alice on `ToChain`.
4. To receive Alice's `x` tokens on `FromChain`, Bob calls `claimFromChainBatch(b)`. Bob can only do this after `BatchAuthorizationTime + SafetyWindow` time. `t` is now in the the `claimed` state.
...
2. Bob can authorize `t` in a batch `b` on `FromChain`, but submit a different batch `b'` on `ToChain`. This is an attributable fault, since Bob promised he would only submit `b`. In this case, Alice can call `proveFraud(b')`, which sends `x` tokens back to Alice on `FromChain` and moves `t` into the `withdrawn` state.
...
- Users (Alice) and liquidity providers (Bob) are **_able to_** observe events on `ToChain` in at most `t_observation_1` time and on `FromChain` in at most `t_observation_2` time.
- Users (Alice) and liquidity providers (Bob) are **_able to_** submit and get their transaction mined on `ToChain` in at most `t_submission_1` time and on `FromChain` in at most `t_submission_2` time.
- Nobody can forge signatures.

## Safety

1. If Alice successfully deposits `x` tokens on `FromChain`, then Alice can guarantee that
   - either Alice reclaims `x` tokens on `FromChain`
   - or Alice receives `x` tokens on `ToChain`
2. If Bob authorizes a ticket with amount `x` on `FromChain`, then Bob can guarantee that he can receive `x` tokens on `FromChain`.
...
1. If Alice successfully deposits `x` tokens on `FromChain`, then Alice can guarantee that `x` tokens are sent to an address provided by Alice, either on `ToChain` or `FromChain`, in time at most `t_access_alice`.
2. If Bob has deposited `x` tokens on `ToChain`, he can guarantee access to a total of `x` tokens across `ToChain` and `FromChain` in time at most `t_access_bob`.
...
### 1. Alice locks up funds on `FromChain` in escrow.

Alice supplies the amount she wishes to swap, as well as some information about `ToChain` amounts that `FromChain` can trust _when putting her ticket in the queue._ Essentially, she is asserting "I believe that there are at least `trustedAmount` tokens available on `ToChain` for tickets with nonce greater than `trustedNonce`." (If Alice submits an unsafe `trustedAmount`, she risks giving some funds to Bob. However, she does not risk another user Amy's funds, since the value she submits does not affect Amy's safety checks.)
...
function depositOnFromChain(FromChainDeposit calldata deposit) public payable {
...
### 2. Bob authorizes withdrawals on `FromChain`
...
Suppose Bob authorized one of Alice’s tickets `t` in a batch `b`. When `t` was registered, the `FromChain` contract made sure that the tickets ahead of Alice would not drain the `ToChain` contract before paying out `t` in full. Since the `ToChain` contract’s funds can _only go down_ by submitting a batch `b'` of tickets signed by Bob, when Alice’s ticket gets registered, it’s either the case that:

- The batch `b` is submitted, and Alice receives `t.amount` tokens on `ToChain`
- Bob signed and submitted a different batch `b' != b`. From `FromChain`’s point of view, this is an attributable fault, and when given proof of such a fault, tickets are refunded on `FromChain`.

**_This is the key fact that makes SAFE safe._**

### 3. Anyone calls `claimBatch` on `ToChain`
    ...
    public
{
    ...
    for (uint256 i = 0; i < tickets.length; i++) {
        tickets[i].toChainRecipient.call{
...
### 4. Bob claims his funds on `FromChain`.

We force Bob to wait `SafetyWindow` time before he can claim his `FromChain` funds. This allows any user to ensure that the correct batch is submitted on `ToChain`. ⚠️If Bob submits an incorrect batch, users must rescue their funds before the `SafetyWindow` passes.⚠️
...
### 5. Refunding on `FromChain` after (provable) fraud

When Bob calls `authorizeWithdrawal`, he is enabling anyone to claim a specific batch of tickets on `ToChain`.
...
Because `FromChain` has recorded exactly which batch he claimed he would submit, this is an attributable fault on `FromChain`! This would let Alice reclaim her escrowed funds. A simple modification of the protocol could penalize Bob for misbehaviour, and compensate Alice for her frustration.
...
### 6. Alice reclaims escrow on `FromChain`
...
function refund(uint256 index) public {
    ...
    tickets[index].toChainRecipient.call{
...
1. Bob _authorizes_ `ticket` in a batch `b` in step 2, before `AuthWindow` passes. This blocks Alice from triggering A6. At this point, the _only batch that can be legally claimed on `ToChain`_ is `b` itself. Anyone can call `claimBatch(b)`, because Bob signed `b` and submitted it to `FromChain`, so Alice can trigger [A3] if Bob does not submit [B3] himself.
   1. If Bob does not sign a different batch `b_bad`, then nobody can call `claimBatch` with `b_bad`. Therefore, any transaction calling `claimBatch(b)` will succeed. Alice can submit this transaction, and can therefore receive her funds on `ToChain`. Bob can receive Alice's `FromChain` tokens at step 4 (B4).
   2. If Bob signs and submits a different batch `b_bad` in [B3'], then anyone can point out that Bob signed `b_bad` _after committing to only submitting `b`._ This act enables anyone to call `refundOnFraud`. In particular, Alice can trigger step 5, [A5] in the diagram below, which returns `x` tokens to Alice on `FromChain`.
2. Bob _does not authorize `ticket` in a batch `b` in step 2._

   If Bob does not authorize `ticket`, then after `AuthWindow`, Alice can point out on `FromChain` that `ticket` has not been authorized, by observing that the latest ticket authorized has nonce less than `ticket.nonce`. (Remember, the `FromChain` rules dictate that the _next batch authorized_ must start with the next ticket not-yet-authorized. Ie. there are no "gaps" in the set of authorized tickets.)

   In short, the rules say Bob must to authorize tickets within `AuthWindow`, and he failed to do so, `FromChain` releases the funds to Alice.
...
- `t_submission_1` time for Bob to submit the batch on `ToChain`. (Note that it's safe for Bob to submit the batch on `ToChain` in [B3] in parallel with authorizing tickets on `FromChain` in (B2).)

So, for Alice `t_happy_path < batch_window + t_submission_1`.

![**Happy path** — Bob submits the correct batch on `ToChain` (B3), but Alice has the option to submit the batch.](img/safe-happy-path.jpg)

**Happy path** — Bob submits the correct batch on `ToChain` (B3), but Alice has the option to submit the batch.
...
- Bob them submits an incorrect batch of tickets `b'` to `ToChain` in [B3']. This must have happened within `t_observation_2 + t_submission_1` time, because Alice would presumably immediately submit `b` after seeing it posted to `FromChain`.
...
Note that this proof depends on the following inequality: `SafetyWindow > t_observation_2 + t_submission_1 + t_observation_1 + t_submission_2`. This prevents Bob from submitting an incorrect batch `b'` to `ToChain` in [B3'], then claiming `FromChain` funds as though the batch `b` were submitted in [B3'].
...
**(S2 + FromChain: Bob can be made whole quickly)**
...
Once the ticket is registered, he can authorize a batch including that ticket (`t_submission_2`), wait `SafetyWWindow` time, and call `claimFromChainFunds`, another `t_submission_2` time.

In parallel, he must also claim the batch on `ToChain`, requiring `t_submission_1` time.
...
Since ORU transactions incur an `ToChain` gas cost, we would like to estimate the total user cost of SAFE by calculating the average amount of calldata required per swap in the `FromChain` transactions.
