...
describe(`L2 Contract`, () => {
  ...
  const transferTypes: TransferType[] = ["ERC20", "ETH"];
  for (const transferType of transferTypes) {
    it(`allows funds to be refunded after claim expiry using ${transferType}`, async () => {
      const entry: EscrowEntry = {
        ...
        claimStart: 0,
        token:
          transferType === "ERC20" ? tokenContract.address : ETH_TOKEN_ADDRESS,
      };
    ...
    it(`allows funds to be claimed after claim start using ${transferType}`, async () => {
    ...
    it(`doesn't allow funds to be claimed after claim expiry using ${transferType}`, async () => {
      const entry: EscrowEntry = {
        value: transferAmount,
        receiver: bobWallet.address,
        sender: aliceWallet.address,
    ...
    it(`allows for fraud to be proven using ${transferType}`, async () => {
    ...
    it(`shouldn't allow proof of fraud with identical commitments  using ${transferType}`, async () => {
      const ticket: Ticket = {
        senderNonce: 1,
        value: transferAmount,
        receiver: aliceWallet.address,
